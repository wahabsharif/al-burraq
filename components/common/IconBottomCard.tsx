...
const IconBottomCard = ({ headerIcon, text, link, footerIcon }: Proptypes) => {
  ...
  const handleMouseEnter = () => {
    ref.current?.classList.add("text-darkGold");
  };
  // !Handle When Someone Enter Mouse Over Card --------------------------->

  // ?Handle When Someone Leave Mouse From Card --------------------------->
  const handleMouseLeave = () => {
    ref.current?.classList.remove("text-darkGold");
  };
  // !Handle When Someone Leave Mouse From Card --------------------------->
  ...
  return (
    <Link href={link}>
      <div
        className="shadow-xl relative flex flex-col gap-8 hover:text-white items-center transition-all hover:bg-darkGold cursor-pointer justify-center rounded-md py-16"
